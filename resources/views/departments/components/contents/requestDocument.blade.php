        ...
        <div class="col-xl-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="mb-2">
                        <a class="filter-button text-white font-size-13 btn btn-info p-1" data-filter="all"
                            data-bs-toggle="tooltip" data-bs-placement="top" title="All Document">All Documents</a>
                        ...
                        <a class="filter-button text-white font-size-13 btn btn-success p-1" data-filter="completed"
                            data-bs-toggle="tooltip" data-bs-placement="top" title="Completed Document">Completed</a>
                    </div>
                    {{-- {{ $logs }} --}}
                                    ...
                                    @if (in_array(Auth::user()->id, $document['destination']))
                                        <tr data-requestor-trk="{{ $document['trk_id'] }}">
                                                    ...
                                                    @case(null)
                                                        @if ($document['status'] !== 'archived')
                                                            <h6 class="mb-0 text-warning"><i
                                                                    class="ri-checkbox-blank-circle-fill font-size-10 text-warning align-middle me-2"></i>{{ __('Pending') }}
                                                            </h6>
                                                        @else
                                                            <h6 class="mb-0 text-danger"><i
                                                                    class="ri-checkbox-blank-circle-fill font-size-10 text-danger align-middle me-2"></i>{{ __('rejected') }}
                                                            </h6>
                                                        @endif
                                                        ...
                                                        <h6 class="mb-0 position-relative">
                                                            ...
                                                            @switch($document['type'])
                                                                @case('my document')
                                                                    ...
                                                                    TRK-{{ $document['trk_id'] }}
                                                                    <span
                                                                        class="position-absolute bottom-50 left-100 translate-middle badge bg-info">
                                                                        {{ $document['type'] }}
                                                                    </span>
                                                                @break
                                                                ...
                                                                @case('requested')
                                                                    @if ($document['status'] !== 'forwarded')
                                                                        ...
                                                                        TRK-{{ $document['trk_id'] }}
                                                                        <span
                                                                            class="position-absolute bottom-50 left-100 translate-middle badge bg-danger">
                                                                            {{ $document['type'] }}
                                                                        </span>
                                                                    @else
                                                                        <i
                                            ...
                                            <td>
                                                ...
                                                <a class="position-relative track-document"
                                                    data-id="{{ $document['document_id'] }}"
                                                    data-trk="{{ $document['trk_id'] }}" data-bs-toggle="tooltip"
                                                    data-bs-placement="top" title="Track document...">
                                                    <span
                                                        class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger"><b><i
                                                                class="fas fa-route"></i></b></span>
                                                </a>
                                                ...
                                                <div id="{{ $uniqueId }}">
                                                    @foreach ($uniqueBadges as $badge)
                                                        @if ($loop->index < $maxBadgesToShow)
                                                            <span
                                                                class="badge bg-info p-1"><b>{{ $badge }}</b></span>
                                                        @endif
                                                    @endforeach
                                                    ...
                                                    @if ($remainingBadges > 0)
                                                        <a href="#" class="position-relative"
                                                            ...
                                                            title="+{{ $remainingBadges }} more...">
                                                            <span
                                                                class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger"><b>+{{ $remainingBadges }}</b></span>
                                                        </a>
                                                    @endif
                                                </div>
                                            </td>
                                                ...
                                                <span class="">
                                                    @if (Auth::user()->assigned != 'viewing' &&
                                                        ...
                                                        <a class="ri-map-pin-line text-white font-size-18 btn btn-danger p-2 pin-document-btn"
                                                            ...
                                                            data-current-loc="{{ $document['current_location'] }}"
                                                            data-scanned-id="{{ $document['scanned'] }}"
                                                            data-trk="{{ $document['trk_id'] }}"
                                                            data-id="{{ $document['document_id'] }}"
                                                            data-document-id="{{ $document['documents'] }}"
                                                            data-office-id="{{ $document['corporate_office']['office_id'] }}"
                                                            data-bs-toggle="tooltip" data-bs-placement="top"
                                                            title="Forward Document"></a>
                                                    @endif
                                                    {{-- for barcodes --}}
                                                    @if ($document['type'] === 'my document' && $document['status'] !== 'pending' && $document['status'] !== 'archived')
                                                        <a class="ri-barcode-line text-white font-size-18 btn btn-dark p-2 barcode-document-btn"
                                                            data-trk="{{ $document['trk_id'] }}" data-bs-toggle="tooltip"
                                                            data-bs-placement="top" title="Print Barcode"></a>
                                                    @endif

                                                    <a id="view-document-btn"
                                                        class="ri-eye-line text-white font-size-18 btn btn-info p-2 view-document-btn"
                                                        data-stats="{{ $document['status'] }}"
                                                        data-po="{{ $document['po'] }}"
                                                        data-amount="{{ $document['amount'] }}"
                                                        data-id="{{ $document['document_id'] }}"
                                                        data-type="{{ $document['belongsTo'] }}"
            ...
            $('#new-request').on('click', function() {
                ...
                $.each(departmentJson, function(officeAbbrev, officeData) {
                    ...
                    $.each(officeData.users, function(index, user) {
                        html +=
                            `<option value="${office.office_abbrev}|${office.office_name}|${user.name}|${user.id}">${office.office_abbrev} - ${user.name}</option>`
